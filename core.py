...
@bot.tree.command(name="info", description = "Responds with infos about FridgeFund",  guild=discord.Object(id=common.SERVER_ID))
async def infoCmd(inter : discord.Interaction):
    if not check_channels(inter, [common.DEFAULT_CHANNEL_ID]) or not check_roles(inter, []):
        return await inter.response.send_message(embed=create_embed(None, common.gInvalidCmdMsg))
    await inter.response.send_message(embed=create_embed(None, "I'm FridgeFund, a bot that manages NPC fridge; type / and look at different commands in discord"))
...
@bot.tree.command(name="item", description = "Responds with info about specific item given as parameter",  guild=discord.Object(id=common.SERVER_ID))
...
@bot.tree.command(name="buy", description = "Buy a certain quantity from certain snack",  guild=discord.Object(id=common.SERVER_ID))
...
async def buyCmd(inter : discord.Interaction, item: str, quantity : int = 1):
    ...
    if not common.DB.user_exist_by_id(inter.user.id):
        return await inter.response.send_message(embed=create_embed(None, "You aren't yet a FridgeFunder, contact admins to become one!"))
    item  = item.lower().strip()
...
@bot.tree.command(name="zwallet", description = "admin: Responds with details about wallet of certain user",  guild=discord.Object(id=common.SERVER_ID))
...
@bot.tree.command(name="zbuy", description = "admin: Buy an item for a certain user",  guild=discord.Object(id=common.SERVER_ID))
...
@bot.tree.command(name="zuser", description = "admin: Create user if not created or update him",  guild=discord.Object(id=common.SERVER_ID))
...
async def zuserCmd(inter : discord.Interaction, user : discord.Member, balance : float = 0.0, first_name : str = ""):
    ...
    if not common.DB.user_exist_by_id(user.id): 
        ...
        await inter.response.send_message(embed=create_embed("Database Info", f"The user has been added to the database."))
        return
...
@bot.tree.command(name="zitem", description = "admin: Create or update an item",  guild=discord.Object(id=common.SERVER_ID))
...
async def zitemCmd(inter : discord.Interaction, item : str, price : float = -1.0, quantity : int =  -1):
    ...
    if not common.DB.item_exist_by_name(item):
        if price == -1 or quantity == -1:
            return await inter.response.send_message(embed=create_embed("Storage","You need to set quantity and price if you add the item for the first time!"))
